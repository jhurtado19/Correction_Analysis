% this function is built to handle disrtibutions that are close to the
...
function [stdTOD, mTOD] = normalcircpeaks(y_vec, t_vec, color, linespec)
   ...
   % z = zscore(pks);
    ...
    x = linspace(0, 48, 1000); % 0 to 36 hours
    y = normpdf(x, mu_final + 24, sigma);  % Wrap x for correct normal shape
    ...
    % adjust mTOD for plotting 
    adjusted_mTOD = mTOD + hours(24);
    % dont need this 

    % Annotate the mean with a red dashed line
    xline(adjusted_mTOD, '--', char(mTOD), ...
        'Color', color, ...
        'LineWidth', 2, ...
    ...
    max_hours = 48; % Maximum number of hours
